...
public class RandomBiomeFruitDecorator extends CocoaDecorator {
	public static final Codec<RandomBiomeFruitDecorator> CODEC = Codec.unit(RandomBiomeFruitDecorator::new);
	...
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, new ResourceLocation("trademine_item_random:random_biome_tree_fruit_decorator"), () -> DECORATOR_TYPE);
	}

	public RandomBiomeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(TrademineItemRandomModBlocks.RANDOM_LEAVES.get().defaultBlockState(), direction1));
						}
					}
				}
			});
