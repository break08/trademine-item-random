...
public class RecipePotionOfInfestedMobBrewingRecipe implements IBrewingRecipe {
	...
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new RecipePotionOfInfestedMobBrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(Items.ROTTEN_FLESH).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(Items.POTION, TrademineItemRandomModPotions.POTION_OF_INFESTED_MOB);
		}
